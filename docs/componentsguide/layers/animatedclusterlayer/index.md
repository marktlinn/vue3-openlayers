...
In case your map doesn't render correctly, interactive elements are blocking, or your are ending up with memory leaks, it's probably worth switching from passing the features directly to requesting features from a URL.
...
The z-index for layer rendering. When rendered, the layers will be ordered, first by Z-index and then by position.
